...
export default function StatusTimeline({ entries, className }: StatusTimelineProps) {
  if (entries.length === 0) {
    return (
      <p className="text-sm text-muted-foreground py-2">No status history</p>
    );
  }
        ...
        {sorted.map((entry) => {
          ...
          return (
            <div key={entry.id} className="relative flex gap-3 pb-4 last:pb-0">
              <div className="absolute left-[-9px] top-2 h-3 w-3 rounded-full border-2 border-primary bg-background" />
